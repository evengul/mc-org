...
| `GetServerFileStep` timeout | version + the configured budgets | **Accepted** (MCO-346, reworded MCO-552). No exception message at all since the download moved onto the shared Ktor client: a mid-body stall arrives as a chain of `ClosedByteChannelException`s with the socket timeout at the root, and none of their messages says anything a fixed line does not. The generic branch still attaches the throwable for a non-timeout failure — a Mojang CDN URL and a Ktor stack, no user data. |
