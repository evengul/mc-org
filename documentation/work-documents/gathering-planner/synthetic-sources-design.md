...
| `water` | `synthetic/ice.json` | BLOCK | — | 1 |
| `lava` | `synthetic/lava.json` | COLLECT | — | 1 |

The 16 concrete entries are generated by looping the dye-colour list, not hand-written.
`water` has two terminals: fill a bucket from a pool (COLLECT) and break naturally-occurring
ice (BLOCK, no input — ice in cold biomes melts to water when broken without silk touch).
Both score ~100 and group under GATHER; the planner picks one.
...
- Note 6 (concrete): `white_concrete` gets a GAME_MECHANIC source consuming concrete powder,
  instead of only self-break — **requires the self-block tweak below to actually win.**
- Note 5 (water): `water` becomes a COLLECT/BLOCK terminal instead of BLOCKED.

### Self-block-loot penalty must recognise GAME_MECHANIC (scorer sign-off)

`white_concrete` already has a self-break loot source (`blocks/white_concrete.json`, BLOCK
100) — "re-collect the block you placed", exactly what the self-block-loot penalty (−200) is
for. But that penalty only fires when the item has a **recipe sibling**
(`hasRecipeSibling = candidates.any { it.sourceType.isRecipe() }`), and `GAME_MECHANIC` is
not a recipe. So without a change, the un-penalised self-break (100) beats the concrete
mechanic (~75) and the plan stays "break placed concrete".

**Fix (flagged, restricted area):** broaden the penalty trigger from "has a recipe sibling"
to "has a **constructive** sibling" — a recipe *or* a `GAME_MECHANIC` source — in
`PlanSelector.rank` (the `hasRecipeSibling` it computes) and the matching check in
`acquirable`. On-intent: a `GAME_MECHANIC` is a deliberate way to make the item, so breaking
the placed block is just as circular as when a recipe exists. Affects any future
GAME_MECHANIC item that also drops itself; no weight values change. This is the only scorer
change in A beyond the two new base scores.
