...
import app.mcorg.organizer.presentation.rest.mappers.project.CreateProjectInputMapper;
import app.mcorg.organizer.presentation.rest.mappers.project.GetAllProjectsOutputMapper;
...
public class ProjectController implements ProjectResource {
    ...
    @Override
    public CompletableFuture<ResponseEntity<GenericResponse>> createFromMaterialList(String name, MultipartFile file) {
        return executor.execute(
                createProjectFromMaterialListUseCase,
                new CreateProjectFromMaterialListUseCase.InputValues(name, file),
                outputValues -> ResponseEntity.ok(new GenericResponse(true, "Project created successfully"))
        );
    }
