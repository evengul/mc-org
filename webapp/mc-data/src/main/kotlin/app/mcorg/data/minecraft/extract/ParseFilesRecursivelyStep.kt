...
abstract class ParseFilesRecursivelyStep<T> : Step<Pair<MinecraftVersion.Release, Path>, ExtractionFailure, List<T>> {
    internal lateinit var version: MinecraftVersion.Release
    internal lateinit var basePath: Path

    private val logger = LoggerFactory.getLogger(javaClass)

    override suspend fun process(input: Pair<MinecraftVersion.Release, Path>): Result<ExtractionFailure, List<T>> {
    ...
    fun extractFilePaths(): Result<ExtractionFailure, List<Path>> {
        return try {
    ...
    abstract suspend fun parseFile(content: String, filename: String): Result<ExtractionFailure, T>
}
