...
fun LI.projectResourceGatheringItem(worldId: Int, projectId: Int, item: ResourceGatheringItem, hasPlan: Boolean = false) {
    ...
    div("project-resources-collection-summary-actions") {
        when (val solvedByProject = item.solvedByProject) {
            null -> ghostButton(if (hasPlan) "Update Resource Path" else "Select Resource Path") {
                ...
                href = Link.Worlds.world(worldId).project(solvedByProject.first).to
                + "Farmable with ${solvedByProject.second}"
            }
        }
