...
class DefaultApiProviderTest {
    ...
    fun `HTTP error should include status code and body`() {
        val httpError = AppFailure.ApiError.HttpError(400, "Bad Request")
    ...
    fun `HTTP error can have null body`() {
        val httpError = AppFailure.ApiError.HttpError(500)
    ...
    fun `fake provider configuration should store response function`() {
        val config = TestApiConfig()
        val responseFunction: (HttpMethod, String) -> Result<AppFailure.ApiError, String> = { _, _ ->
        ...
        assertEquals(ApiConfig.ProviderType.FAKE, config.provider)
        assertIs<Function2<HttpMethod, String, Result<AppFailure.ApiError, String>>>(config.fakeResponses)
