...
object GetProfileStep : Step<Int, DatabaseFailure, Profile> {
    override fun process(input: Int): Result<DatabaseFailure, Profile> {
        return useConnection {
            val resultSet = prepareStatement("select id,username,email,profile_photo,selected_world,technical_player from users where id=?")
                .apply { setInt(1, input) }
                .executeQuery()
            if (resultSet.next()) {
                return@useConnection Result.success(Profile(
                    resultSet.getInt(1),
