...
object ShapedRecipeParser {
    ...
    suspend fun parse(json: JsonElement, filename: String): Result<AppFailure, List<ResourceSource>> {
        val craftingValues = CraftingValuesParser.parse(json, filename)

        if (craftingValues is Result.Failure) {
            logger.warn("Shaped recipe missing ingredients or result id in $filename")
