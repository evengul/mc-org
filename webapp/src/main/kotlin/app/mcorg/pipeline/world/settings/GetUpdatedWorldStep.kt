...
object GetUpdatedWorldStep : Step<Int, UpdateWorldNameFailures, World> {
    override suspend fun process(input: Int): Result<UpdateWorldNameFailures, World> {
        ...
        return DatabaseSteps.query<Int, UpdateWorldNameFailures, World?>(
            sql = getWorldQuery,
            parameterSetter = { statement, _ ->
                statement.setInt(1, input)
            },
            errorMapper = { UpdateWorldNameFailures.DatabaseError(it) },
            resultMapper = { if (it.next()) it.toWorld() else null }
        ).process(input).flatMap { world ->
            if (world != null) {
                Result.success(world)
            } else {
                Result.failure(UpdateWorldNameFailures.DatabaseError(
