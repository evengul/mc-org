...
class UnitCostModelAdversarialTest {
    ...
    /**
     * **Defect 5 — `best` has no declared tie-break.**
     *
     * `minByOrNull` returns the first minimum in `getSourcesForItem`'s iteration order, which
     * comes from the graph build order, which comes from `resource_source.id`. 91 items in world
     * 3 have an exact tie for cheapest, and about 42 of the 158 reported disagreements with the
     * shipped model are nothing but this — two block-loot sources both at 0.05 min. `PlanSelector`
     * pins its own order (score, then recipe-first, then source key); the cost model does not, so
     * those picks would move on a re-ingest.
     */
    @Test
    fun `two sources at the same cost are separated only by graph insertion order`() {
        ...
        assertTrue(
            UnitCostModel(forward).best(dandelion)?.getKey() != UnitCostModel(reversed).best(dandelion)?.getKey(),
            "but the pick follows insertion order"
        )
    }
}
